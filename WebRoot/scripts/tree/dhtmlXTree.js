...
function dhtmlXTreeObject(htmlObject, width, height, rootId) {
	...
	this.actionNode = new dhtmlXTreeItemObject(this.rootId, "", 0, this);
	this.actionNode.actionNode.childNodes[0].childNodes[0].style.display = "none";
	this.actionNode.actionNode.childNodes[0].childNodes[0].childNodes[0].className = "hiddenRow";
	this.allTree = this._createSelf();
	this.allTree.appendChild(this.actionNode.actionNode);
...
function dhtmlXTreeItemObject(itemId, itemText, parentObject, treeObject, actionHandler, mode) {
	...
	if (this.treeNod.checkBoxOff)this.actionNode = this.treeNod._createItem(1, this, mode); else this.actionNode = this.treeNod._createItem(0, this, mode);
	this.actionNode.objBelong = this;
	return this;
}
;
...
dhtmlXTreeObject.prototype._attachChildNode = function(parentObject, itemId, itemText, itemActionHandler, image1, image2, image3, optionStr, childs, beforeNode, afterNode) {
    ...
    if(parentObject.id=='0'){
        itemText="<span class='tree_head'>"+itemText+"</span>";
    }
	...
	var tr = this._drawNewTr(Nodes[Count].actionNode);
	if ((this.XMLloadingWarning) || (this._hAdI))
		Nodes[Count].actionNode.parentNode.parentNode.style.display = "none";
	if ((beforeNode) && (beforeNode.tr.nextSibling))
		parentObject.actionNode.childNodes[0].insertBefore(tr, beforeNode.tr.nextSibling); else
		if (this.parsingOn == parentObject.id) {
			this.parsedArray[this.parsedArray.length] = tr;
		} else
			parentObject.actionNode.childNodes[0].appendChild(tr);
	...
	if ((parentObject._r_logic) || (this._frbtr))
		this._setSrc(Nodes[Count].actionNode.childNodes[0].childNodes[0].childNodes[1].childNodes[0], this.imPath + this.radioArray[0]);
	if (optionStr) {
		var tempStr = optionStr.split(",");
		for (var i = 0; i < tempStr.length; i++) {
			switch (tempStr[i])
...
dhtmlXTreeObject.prototype._parseXMLTree = function(dhtmlObject, node, parentId, level, xml_obj, start) {
	...
	if (!node) {
		...
		if (node.getAttribute("radio"))
			dhtmlObject.actionNode._r_logic = true;
	...
	if (!level) {
		...
		for (var i = 0; i < dhtmlObject.parsedArray.length; i++)parsedNodeTop.actionNode.childNodes[0].appendChild(dhtmlObject.parsedArray[i]);
...
dhtmlXTreeObject.prototype._redrawFrom = function(dhtmlObject, itemObject, start, visMode) {
	...
	for (var i = (start ? start - 1 : 0); i < tempx.childsCount; i++)
	{
		if ((!this._branchUpdate) || (this._getOpenState(tempx) == 1))
			if ((!itemObject) || (visMode == 1)) tempx.childNodes[i].actionNode.parentNode.parentNode.style.display = "";
...
dhtmlXTreeObject.prototype._xcloseAll = function(itemObject)
{
	...
	if (this.rootId != itemObject.id) {
		var Nodes = itemObject.actionNode.childNodes[0].childNodes;
...
dhtmlXTreeObject.prototype._correctPlus = function(itemObject) {
	var imsrc = itemObject.actionNode.childNodes[0].childNodes[0].childNodes[0].lastChild;
	var imsrc2 = itemObject.actionNode.childNodes[0].childNodes[0].childNodes[2].childNodes[0];
	...
	if ((this.XMLsource) && (!itemObject.XMLload))
	{
		...
		{
			if ((itemObject.actionNode.childNodes[0].childNodes[1]) && ( itemObject.actionNode.childNodes[0].childNodes[1].style.display != "none" ))
			{
...
dhtmlXTreeObject.prototype._correctLine = function(itemObject) {
	...
	if (sNode)if ((this._getLineStatus(itemObject.id, sNode) == 0) || (!this.treeLinesOn))
		for (var i = 1; i <= itemObject.childsCount; i++) {
			itemObject.actionNode.childNodes[0].childNodes[i].childNodes[0].style.backgroundImage = "";
			itemObject.actionNode.childNodes[0].childNodes[i].childNodes[0].style.backgroundRepeat = "";
		} else
		for (var i = 1; i <= itemObject.childsCount; i++) {
			itemObject.actionNode.childNodes[0].childNodes[i].childNodes[0].style.backgroundImage = "url(" + this.imPath + this.lineArray[5] + ")";
			itemObject.actionNode.childNodes[0].childNodes[i].childNodes[0].style.backgroundRepeat = "repeat-y";
		}
	;
};
...
dhtmlXTreeObject.prototype._getOpenState = function(itemObject) {
	var z = itemObject.actionNode.childNodes[0].childNodes;
...
dhtmlXTreeObject.prototype._markItem = function(node) {
	...
	if(node.parentObject.id==0 || node.type.substr(0,2)=="0_"){
        if(node.type.substr(0,5)=="guide"){
            node.span.className = "selectedTreeRow";
        }else{
           node.span.className = "selectedTreeRow0";
        }
    }else{
        node.span.className = "selectedTreeRow";
    }
...
dhtmlXTreeObject.prototype.onMouseOver = function () {
    if(this.parentObject.parentObject.id!=0 && this.parentObject.type.substr(0,2)!="0_" ||this.parentObject.type.substr(0,5)=="guide"){
	   this.className="standartTreeRow_lor";
    }
 }
dhtmlXTreeObject.prototype.onMouseOut = function () {
    if(this.parentObject.parentObject.id!=0 && this.parentObject.type.substr(0,2)!="0_" ||this.parentObject.type.substr(0,5)=="guide"){
	   this.className="standartTreeRow";
    }
}
dhtmlXTreeObject.prototype._createItem = function(acheck, itemObject, mode) {
    ...
    td2.onmouseover = this.onMouseOver;
    td2.onmouseout = this.onMouseOut;
    ...
    if(itemObject.parentObject.id==0){
        if(itemObject.type.substr(0,5)=="guide"){
            td2.style.cursor = this.style_pointer;
        }else{
            td2.style.cursor = "text";
        }
    }else if(itemObject.type!=0){
        if(itemObject.type.substr(0,2)=="0_"){
           td2.style.cursor = "text";
        }else{
           td2.style.cursor = this.style_pointer;
        }
...
dhtmlXTreeObject.prototype.setUserData = function(itemId, name, value) {
	...
	if (!sNode)return;
	if (name == "hint")sNode.actionNode.childNodes[0].childNodes[0].title = value;
...
dhtmlXTreeObject.prototype.getItemText = function(itemId)
{
	var temp = this._globalIdStorageFind(itemId);
	if (!temp)return 0;
	return(temp.actionNode.childNodes[0].childNodes[0].childNodes[3].childNodes[0].innerHTML);
};
...
dhtmlXTreeObject.prototype._moveNodeTo = function(itemObject, targetObject, beforeNode) {
	...
	} else
	{
		...
		var tr = this._drawNewTr(Nodes[Count].actionNode);
		if (!beforeNode) {
			targetObject.actionNode.childNodes[0].appendChild(tr);
			if (this.dadmode == 1)this._fixNodesCollection(targetObject, beforeNode);
		} else
		{
			targetObject.actionNode.childNodes[0].insertBefore(tr, beforeNode.tr);
	...
	if ((!oldTree.dpcpy) && (!framesMove)) {
		...
		if ((document.all) && (navigator.appVersion.search(/MSIE\ 5\.0/gi) != -1))
		{
			...
			itemObject.parentObject.actionNode.childNodes[0].removeChild(itemObject.tr);
...
dhtmlXTreeObject.prototype._clearStyles = function(itemObject) {
	var td1 = itemObject.actionNode.childNodes[0].childNodes[0].childNodes[1];
...
dhtmlXTreeObject.prototype._openItem = function(item) {
	...
	if ((state < 0) || (((this.XMLsource) && (!item.XMLload)))) {
		...
		if (this._epnFH) {
			...
			if (!this.xmlstate)this._epnFH(item.id, this._getOpenState(item)); else {
...
dhtmlXTreeObject.prototype.selectItem = function(itemId, mode, preserve) {
	...
	;
	if (mode)this.onRowSelect(ze, temp.actionNode.childNodes[0].childNodes[0].childNodes[3], false); else
		this.onRowSelect(ze, temp.actionNode.childNodes[0].childNodes[0].childNodes[3], true);
};
...
dhtmlXTreeObject.prototype._deleteNode = function(itemId, htmlObject, skip) {
	...
	for (var i = 0; i < Count; i++) {
		if (Nodes[i].id == itemId) {
			if (!skip)sN.actionNode.childNodes[0].removeChild(Nodes[i].tr);
			Nodes[i] = 0;
			break;
		}
		;
...
dhtmlXTreeObject.prototype._setCheck = function(sNode, state) {
	...
	var z = sNode.actionNode.childNodes[0].childNodes[0].childNodes[1].childNodes[0];
...
dhtmlXTreeObject.prototype._focusNode = function(item) {
	var z = getAbsoluteTop(item.actionNode) - getAbsoluteTop(this.allTree);
...
dhtmlXTreeObject.prototype._drag = function(sourceHtmlObject, dhtmlObject, targetHtmlObject) {
	...
	if (!targetHtmlObject.parentObject) {
		targetHtmlObject = this.actionNode.actionNode.childNodes[0].childNodes[0].childNodes[1].childNodes[0];
		this.dadmodec = 0;
	}
	;
	this._clearMove();
...
dhtmlXTreeObject.prototype.getAllChildless = function() {
	return this._getAllScraggyItems(this.actionNode);
};
...
dhtmlXTreeObject.prototype.getAllItemsWithKids = function() {
	return this._getAllFatItems(this.actionNode);
};
...
dhtmlXTreeObject.prototype._getAllChecked = function(htmlNode, list, mode) {
	if (!htmlNode)htmlNode = this.actionNode;
